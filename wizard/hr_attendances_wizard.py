...
class AttendancesWizard(models.TransientModel):
    ...
    def check_report(self):
        data = {}
        data['form'] = self.read(['start_date', 'end_date','file_format','employee_id'])[0]
        return self._print_report(data)

    def _print_report(self, data):
        data['form'].update(self.read(['desde', 'hasta','file_format','employee_id'])[0])
        print('HOLA LLEGUE HASTA ACA asani32323u****')
...
class ReportAttendanceWizard(models.AbstractModel):
    _name = 'report.hr_attendance_report.attendance_report'
    @api.model
